...
void Grammar(NFA nfa) {
  std::string filename = "equivalentgrammar.txt";
  std::ofstream output_file(filename);

  // Alfabeto gramática
  output_file << "Alfabeto:" << nfa.getAlphabet() << std::endl;
  
  // Simboos no terminales
  output_file << "Simbolos no terminales:\n";
  for (unsigned i{0}; i < nfa.getStates().size(); ++i) {
    output_file << static_cast<char>('A'+ i) << "\n"; 
  }
  
  // Simbolo de arranque
  output_file << "Simbolo inicial:\n" << static_cast<char>(nfa.getInitial().getID()+'A') << "\n";

  // Producciones
  output_file << "Producciones:\n";
  std::vector<Transition> aux = nfa.getTransitions();
  for (unsigned i{0}; i < nfa.getTransitions().size(); ++i) {
    output_file << static_cast<char>(aux[i].getActualState()+'A') << " -> " << aux[i].getSymbol() << static_cast<char>(aux[i].getNextState()+'A') << std::endl;
