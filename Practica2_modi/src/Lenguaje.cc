...
Lenguaje Lenguaje::concat() {
  Lenguaje result;
  Lenguaje aux = *this;
  Cadena cad1;
  Cadena cad2;
  for (auto it1 = leng_.begin(); it1 != leng_.end(); ++it1) {
    for (auto it2 = aux.leng_.begin(); it2 != aux.leng_.end(); ++it2) {
      cad1 = *it1;
      cad2 = *it2;
      result.insert(cad1.concat(cad2));
    }
  }
...
Lenguaje Lenguaje::exp(unsigned n) {
  Lenguaje result;
  if (n == 0) {
    Cadena chain;
    result.insert(chain);
